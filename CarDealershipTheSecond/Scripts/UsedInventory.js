$("#submitquicksearchbutton").on('click', function () {
    ...
    $.ajax({
        type: "GET",
        url: "https://localhost:44386/searchused?" + query,
        success: function (vehicles) {
            $.each(vehicles, function (index, vehicle) {
                ...
                result += "<button type='button' onclick='GetDetails(\"" + vehicle.VIN + "\")'>Details</button></div ></div></div>";
                result += "</div></div></div>";
...
function GetDetails(VIN) {
    window.location.href = "https://localhost:44386/inventory/details?VIN=" + VIN;
}